...
logFile="${logsDir}/phone.$(date +'%Y-%m-%d').log"
mkdir -p "${logsDir}" "${htmlDir}"
...
provinceInBack () {
  provinceList=(安徽 河北 山西 辽宁 吉林 黑龙江 江苏 浙江 福建 江西 山东 河南 湖北 湖南 广东 海南 四川 贵州 云南 陕西 甘肃 青海 台湾 内蒙古 广西 西藏 宁夏 新疆)
  for province in "${provinceList[@]}"; do
    if [[ "${province}" == "${1}" ]]; then
      echo "1"
      return
    fi
...
getGsd () {
    ...
    if [[ "${test_0}" ]]; then
        ...
        numSize=$(stat "${phoneP}" | grep Size | awk '{ print $2 }')
